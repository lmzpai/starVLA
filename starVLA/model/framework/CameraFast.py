...
@FRAMEWORK_REGISTRY.register("CameraFast")
class CameraFast(baseframework):
    """
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        # --- output text token (index based on VLM) ---
        tok = self.qwen_vl_interface.processor.tokenizer
        pad_id = tok.pad_token_id
        prompt_lens = qwen_inputs["input_ids"].ne(pad_id).sum(dim=1).tolist()
        output_text = [
            tok.decode(generated_ids[b, prompt_lens[b] :], skip_special_tokens=True, clean_up_tokenization_spaces=False)
            for b in range(generated_ids.size(0))
        ]
        print(f"Camera Output Text: {output_text}")
...
if __name__ == "__main__":
    ...
    # debugpy.listen(("0.0.0.0", 11092))
    # print("🔍 Rank 0 waiting for debugger attach on port 10092...")
    # debugpy.wait_for_client()
    args.config_yaml = "/share/project/zhouenshen/hpfs/code/ActivePerception/starVLA/examples/CameraFast/train_files/test.yaml"
    cfg = OmegaConf.load(args.config_yaml)
    # cfg.framework.qwenvl.base_vlm = "./playground/Pretrained_models/Qwen3-VL-4B-Instruct-Action"

    # try get model
    model = CameraFast(cfg)
    print(model)
